...
class AntiIndiaHateDetector:
    """
    ML Pipeline for detecting anti-India hate speech and sentiment analysis
    """
    
    def __init__(self):
        self.model_name = "martin-ha/toxic-comment-model"  # Better for hate speech detection
        self.tokenizer = None
        self.model = None
    ...
    def _load_models(self):
        """Load pre-trained models for hate detection"""
        try:
            logger.info("Loading hate detection models...")
            
            # Load tokenizer and model
            self.tokenizer = AutoTokenizer.from_pretrained(self.model_name)
            self.model = AutoModelForSequenceClassification.from_pretrained(self.model_name)
            
            # Create hate classification pipeline
    ...
    def classify_hate_speech(self, text: str) -> Dict:
        """Classify text for hate speech using pre-trained model"""
        if not self.is_loaded:
            return {"error": "Model not loaded", "confidence": 0.0}
        
        try:
            # Get hate speech classification
            result = self.hate_classifier(text)
            ...
            return {
                "hate_detected": toxic_score > 0.5,
                "hate_confidence": toxic_score * 100,
                "classification_result": result
            }
            
        except Exception as e:
            logger.error(f"Error in hate speech classification: {e}")
    ...
    def _generate_indicators(self, keyword_analysis: Dict, hate_analysis: Dict, text: str) -> List[str]:
        ...
        while len(indicators) < 3:
            indicators.append("Content analysis completed successfully")
...
# Global instance
hate_detector = AntiIndiaHateDetector()